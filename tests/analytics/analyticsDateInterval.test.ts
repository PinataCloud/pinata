import { ValidationError, AuthenticationError, NetworkError, PinataError, PinataConfig, TimeIntervalAnalyticsQuery, TimeIntervalAnalyticsResponse, TimePeriodItem, analyticsDateInterval } from "../../src/";
...
describe("analyticsDateInterval function", () => {
  ...
  it("should throw ValidationError if config is missing", async () => {
    await expect(analyticsDateInterval(undefined)).rejects.toThrow(ValidationError);
  });
  ...
  it("should throw AuthenticationError on 401 response", async () => {
    ...
    await expect(analyticsDateInterval(mockConfig)).rejects.toThrow(AuthenticationError);
  });
  ...
  it("should throw NetworkError on non-401 error response", async () => {
    ...
    await expect(analyticsDateInterval(mockConfig)).rejects.toThrow(NetworkError);
  });
  ...
  it("should throw PinataError on fetch failure", async () => {
    ...
    await expect(analyticsDateInterval(mockConfig)).rejects.toThrow(PinataError);
  });
