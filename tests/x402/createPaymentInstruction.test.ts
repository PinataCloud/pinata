...
describe("createPaymentInstruction function", () => {
	...
	it("should throw ValidationError if config is missing", async () => {
		await expect(
			createPaymentInstruction(undefined, mockCreateRequest),
	...
	it("should throw ValidationError if name is missing", async () => {
		...
		await expect(
			createPaymentInstruction(mockConfig, invalidRequest),
		).rejects.toThrow(ValidationError);
	});
	...
	it("should throw AuthenticationError on 401 response", async () => {
		...
		await expect(
			createPaymentInstruction(mockConfig, mockCreateRequest),
