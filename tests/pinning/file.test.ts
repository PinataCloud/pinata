...
describe("uploadFile function", () => {
  ...
  it("should throw ValidationError if config is missing", async () => {
    await expect(uploadFile(undefined, mockFile)).rejects.toThrow(ValidationError);
  });
  ...
  it("should throw AuthenticationError on 401 response", async () => {
    ...
    await expect(uploadFile(mockConfig, mockFile)).rejects.toThrow(AuthenticationError);
  });
  ...
  it("should throw NetworkError on non-401 error response", async () => {
    ...
    await expect(uploadFile(mockConfig, mockFile)).rejects.toThrow(NetworkError);
  });
