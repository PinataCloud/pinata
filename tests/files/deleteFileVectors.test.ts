...
describe("deleteFileVectors function", () => {
  ...
  it("should throw ValidationError if config is missing", async () => {
    await expect(deleteFileVectors(undefined, "test-file-id")).rejects.toThrow(ValidationError);
  });
  ...
  it("should throw AuthenticationError on 401 response", async () => {
    ...
    await expect(deleteFileVectors(mockConfig, "test-file-id")).rejects.toThrow(AuthenticationError);
  });
  ...
  it("should throw NetworkError on non-401 error response", async () => {
    ...
    await expect(deleteFileVectors(mockConfig, "test-file-id")).rejects.toThrow(NetworkError);
  });
  ...
  it("should throw PinataError on fetch failure", async () => {
    ...
    await expect(deleteFileVectors(mockConfig, "test-file-id")).rejects.toThrow(PinataError);
  });
