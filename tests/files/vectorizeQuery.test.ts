...
describe("vectorizeQuery function", () => {
	...
	it("should throw ValidationError if config is missing", async () => {
		await expect(vectorizeQuery(undefined, mockQuery)).rejects.toThrow(
			ValidationError,
		);
	});
	...
	it("should throw AuthenticationError on 401 response", async () => {
		...
		await expect(vectorizeQuery(mockConfig, mockQuery)).rejects.toThrow(
			AuthenticationError,
		);
	});
	...
	it("should throw PinataError when no matches found with returnFile", async () => {
		...
		await expect(
			vectorizeQuery(mockConfig, { ...mockQuery, returnFile: true }),
