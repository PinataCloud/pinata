...
import type { PinataConfig, FileListItem, FileListQuery, FileListResponse } from "../../src/core/types";
import {
    PinataError,
    NetworkError,
    AuthenticationError,
...
describe("listFiles function", () => {
    ...
    it("should handle all query parameters correctly", async () => {
        ...
        global.fetch = jest.fn().mockResolvedValueOnce({
            ok: true,
            json: jest.fn().mockResolvedValueOnce({ data: { files: [], next_page_token: "" } }),
        });
        ...
        expect(global.fetch).toHaveBeenCalledWith(
            expect.stringContaining(
                "limit=10&pageToken=test-token&cidPending=true"
            ),
            expect.any(Object),
        );
    });
