...
describe("createKey function", () => {
  ...
  it("should create a key successfully", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith("https://api.pinata.cloud/v3/pinata/keys", {
      method: "POST",
      headers: {
  ...
  it("should throw ValidationError if config is missing", async () => {
    await expect(createKey(undefined, mockOptions)).rejects.toThrow(ValidationError);
  });
  ...
  it("should throw AuthenticationError on 401 response", async () => {
    ...
    await expect(createKey(mockConfig, mockOptions)).rejects.toThrow(AuthenticationError);
  });
  ...
  it("should throw NetworkError on non-401 error response", async () => {
    ...
    await expect(createKey(mockConfig, mockOptions)).rejects.toThrow(NetworkError);
  });
  ...
  it("should throw PinataError on unexpected errors", async () => {
    ...
    await expect(createKey(mockConfig, mockOptions)).rejects.toThrow(PinataError);
  });
