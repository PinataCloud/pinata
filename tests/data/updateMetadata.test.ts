...
describe("updateMetadata function", () => {
  ...
  it("should throw ValidationError if config is missing", async () => {
    await expect(updateMetadata(undefined, mockOptions)).rejects.toThrow(ValidationError);
  });
  ...
  it("should throw NetworkError on non-401 error response", async () => {
    ...
    await expect(updateMetadata(mockConfig, mockOptions)).rejects.toThrow(NetworkError);
  });
  ...
  it("should throw PinataError on unexpected errors", async () => {
    ...
    await expect(updateMetadata(mockConfig, mockOptions)).rejects.toThrow(PinataError);
  });
