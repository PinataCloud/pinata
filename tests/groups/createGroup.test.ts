...
describe("createGroup function", () => {
  ...
  it("should create a group successfully", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith("https://api.pinata.cloud/groups", {
      method: "POST",
      headers: {
  ...
  it("should throw ValidationError if config is missing", async () => {
    await expect(createGroup(undefined, mockOptions)).rejects.toThrow(ValidationError);
  });
  ...
  it("should throw NetworkError on non-401 error response", async () => {
    ...
    await expect(createGroup(mockConfig, mockOptions)).rejects.toThrow(NetworkError);
  });
  ...
  it("should throw PinataError on unexpected errors", async () => {
    ...
    await expect(createGroup(mockConfig, mockOptions)).rejects.toThrow(PinataError);
  });
