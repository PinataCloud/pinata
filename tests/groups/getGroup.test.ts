...
import type { PinataConfig, GetGroupOptions, GroupResponseItem } from "../../src";
import {
  PinataError,
  NetworkError,
  AuthenticationError,
...
describe("getGroup function", () => {
  ...
  it("should throw ValidationError if config is missing", async () => {
    await expect(getGroup(undefined, mockOptions)).rejects.toThrow(ValidationError);
  });
  ...
  it("should throw AuthenticationError on 401 response", async () => {
    ...
    await expect(getGroup(mockConfig, mockOptions)).rejects.toThrow(AuthenticationError);
  });
  ...
  it("should throw NetworkError on non-401 error response", async () => {
    ...
    await expect(getGroup(mockConfig, mockOptions)).rejects.toThrow(NetworkError);
  });
  ...
  it("should throw PinataError on unexpected errors", async () => {
    ...
    await expect(getGroup(mockConfig, mockOptions)).rejects.toThrow(PinataError);
  });
  ...
  it("should handle retrieval of non-existent group", async () => {
    ...
    await expect(getGroup(mockConfig, { groupId: "non-existent-id" })).rejects.toThrow(
      NetworkError,
    );
  });
  ...
  it("should handle empty group id", async () => {
    ...
    await expect(getGroup(mockConfig, emptyIdOptions)).rejects.toThrow(PinataError);
  });
