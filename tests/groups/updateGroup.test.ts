...
import type { PinataConfig, UpdateGroupOptions, GroupResponseItem } from "../../src";
import {
  PinataError,
  NetworkError,
  AuthenticationError,
...
describe("updateGroup function", () => {
  ...
  it("should throw ValidationError if config is missing", async () => {
    await expect(updateGroup(undefined, mockOptions)).rejects.toThrow(ValidationError);
  });
  ...
  it("should throw NetworkError on non-401 error response", async () => {
    ...
    await expect(updateGroup(mockConfig, mockOptions)).rejects.toThrow(NetworkError);
  });
  ...
  it("should throw PinataError on unexpected errors", async () => {
    ...
    await expect(updateGroup(mockConfig, mockOptions)).rejects.toThrow(PinataError);
  });
